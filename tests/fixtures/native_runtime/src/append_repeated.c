...
void entry(void) {
    ...
    HANDLE f = CreateFileW(L"repeat.txt", FILE_APPEND_DATA, 0, 0, OPEN_EXISTING, 0x80, 0);
    if (f == (HANDLE)(intptr_t)-1) ExitProcess(1);
    DWORD w=0; WriteFile(f, msg1, sizeof(msg1)-1, &w, 0); CloseHandle(f);
    f = CreateFileW(L"repeat.txt", FILE_APPEND_DATA, 0, 0, OPEN_EXISTING, 0x80, 0);
    if (f == (HANDLE)(intptr_t)-1) ExitProcess(2);
    WriteFile(f, msg2, sizeof(msg2)-1, &w, 0); CloseHandle(f);
    ExitProcess(0);
}
